...
class ProposalGenerator():
    ...
    def __call__(self, anchor, box_reg, obj_score):
        ...
        rpn_rois = torch.concat(rpn_rois)
        rpn_scores = torch.concat(rpn_scores)
...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self,feat_map, gt_boxes=None, gt_label=None):
        '''
        ...
        if self.training:
            targets = self.target_generator(anchors, gt_boxes, gt_label)

            gt_proposals = self.proposal_generator.assignLabels(proposals['boxes'], proposals['scores'], gt_boxes, gt_label, self.num_classes)
            ...
            target_labels = targets['labels'].type_as(gt_label)
